...
describe("basic CRUD (postgres)", () => {
  ...
  it(
    "where with closure variable filters by country",
    async () => {
      const country = "US";
      const rows = await User.query()
  ...
  it(
    "includes string filter (case-sensitive in Postgres)",
    async () => {
      // Postgres LIKE is case-sensitive; "li" matches "Al-i-ce"
      const rows = await User.query()
        .where((u) => u.name.includes("li"))
        .toArray();
      expect(rows).toHaveLength(1);
      expect((rows[0] as any).name).toBe("Alice");
    },
...
describe("entity subclassing and lifecycle hooks (postgres)", () => {
  ...
  it(
    "insert returns hydrated instance with id",
    async () => {
  ...
  it(
    "subclass query returns instances with computed getter",
    async () => {
  ...
  it(
    "findById returns correct instance",
    async () => {
      const alice = await User.query().insert({
        name: "Alice",
        email: "alice@example.com",
  ...
  it(
    "new instance save sets id and beforeSave fires",
    async () => {
  ...
  it(
    "post where published filters correctly",
    async () => {
      const alice = await User.query().insert({
        name: "Alice",
        email: "alice@example.com",
...
describe("non-circular many-to-one relation (postgres)", () => {
  ...
  it(
    "select with partial relation loads company name",
    async () => {
...
describe("non-circular one-to-many relation (postgres)", () => {
  ...
  it(
    "select with oneToMany loads employees per department",
    async () => {
...
describe("circular-style bidirectional relations (postgres)", () => {
  ...
  it(
    "manyToOne: each post loads its author",
    async () => {
  ...
  it(
    "partial relation select omits extra fields",
    async () => {
      const rows = await Post.query()
  ...
  it(
    "oneToMany: each user loads their posts",
    async () => {
      const rows = await User.query()
        .select((u: any) => ({
          id: u.id,
  ...
  it(
    "where + select filters to published posts with author",
    async () => {
      const rows = await Post.query()
...
describe("relation-where: JOIN and EXISTS filtering (postgres)", () => {
  const Company = Entity("pg_scen_rw_companies", {
    id: "SERIAL PRIMARY KEY",
    name: "text not null",
  });
  const Category = Entity("pg_scen_rw_categories", {
    id: "SERIAL PRIMARY KEY",
    name: "text not null",
  });
  const Contact = Entity(
  ...
  const Employee = Entity("pg_scen_rw_employees", {
    id: "SERIAL PRIMARY KEY",
    name: "text not null",
    departmentId: "integer not null",
  });
  ...
  it(
    "where with relation (JOIN): contacts at Acme Corp",
    async () => {
      const rows = await Contact.query()
  ...
  it(
    "select spread + relation: all columns plus company",
    async () => {
      const rows = await Contact.query()
  ...
  it(
    "oneToMany where with some() (EXISTS): departments with Alice",
    async () => {
      const rows = await Department.query()
...
describe("explicit join type hints (postgres)", () => {
  ...
  it(
    "innerJoin excludes orphan rows (no matching author)",
    async () => {
  ...
  it(
    "innerJoin with single-member syntax p => p.author",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "innerJoin + where on joined relation column",
    async () => {
      const rows = await Article.query()
        .innerJoin((a: any) => ({ author: a.author }))
  ...
  it(
    "innerJoin + orderBy on joined relation column",
    async () => {
      const rows = await Article.query()
        .innerJoin((a: any) => ({ author: a.author }))
  ...
  it(
    "leftJoin includes orphan rows (null authorId)",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "leftJoin with single-member syntax p => p.author includes orphans",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "leftJoin + count returns all rows including orphans",
    async () => {
  ...
  it(
    "crossJoin excludes orphan rows (ON condition filters cartesian product)",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "crossJoin with single-member syntax p => p.author excludes orphans",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "rightJoin excludes orphan rows (no matching author in right table)",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "rightJoin with single-member syntax p => p.author excludes orphan rows",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "fullJoin includes all rows from both sides (orphan article retained)",
    async () => {
      const rows = await Article.query()
  ...
  it(
    "fullJoin with single-member syntax p => p.author retains orphan article",
    async () => {
      const rows = await Article.query()
